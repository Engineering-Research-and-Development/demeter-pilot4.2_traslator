...
(function(b) {
	...
	b.fn.uploadFile = function(l) {
		...
		function f(o, m, n) {
			n.on("dragenter", function(p) {
				p.stopPropagation();
				p.preventDefault();
				b(this).css("border", "1px solid #333333")
			});
			...
			n.on("drop", function(q) {
				b(this).css("border", "1px dotted #333333");
				q.preventDefault();
		...
		function h(w, q, m) {
			for ( var r = 0; r < m.length; r++) {
				if (d(q, w, m[r].name)) {
					...
					var n = b("<form style='display:block; position:absolute;left: 150px;' class='"
							+ q.formGroup
							+ "' method='"
							+ w.method
							+ "' action='"
		...
		function j(t, r, o, m) {
			...
			var q = b("<form method='" + o.method + "' action='" + o.url
			...
			if (o.multiple) {
				...
				n = "<input type='file' id='" + u + "' name='" + o.fileName
						+ "' multiple/>"
			}
				...
				if (this.files) {
					for (x = 0; x < this.files.length; x++) {
						if (!d(t, o, this.files[x].name)) {
							alert("File type is not allowed. Allowed only: "
									+ o.allowedTypes);
							return
						}
		...
		function i(m) {
			...
			this.abort = b("<div class='ajax-file-upload-red'>Abort</div>")
					.appendTo(this.statusbar).hide();
			this.cancel = b("<div class='ajax-file-upload-red'>Cancel</div>")
					.appendTo(this.statusbar).hide();
			this.done = b("<div class='ajax-file-upload-green'>Done</div>")
					.appendTo(this.statusbar).hide();
			...
			error : function(k, i, j) {
				...
				if (k.statusText == "abort") {
					...
					if (g.showStatusAfterError) {
						c.progressDiv.hide();
						c.statusbar.append("<font color='red'>ERROR: " + j
								+ "</font>")
					} else {
						c.statusbar.hide()
					}
	...
	if (b.fn.ajaxForm == undefined) {
		/*
		 * ! jQuery Form Plugin version: 3.40.0-2013.08.13 @requires jQuery v1.5
		 * or later Copyright (c) 2013 M. Alsup Examples and documentation at:
		 * http://malsup.com/jquery/form/ Project repository:
		 * https://github.com/malsup/form Dual licensed under the MIT and GPL
		 * licenses. https://github.com/malsup/form#copyright-and-license
		 */
		;
		(function(h) {
			var e = {};
			...
			h.fn.ajaxSubmit = function(m) {
				...
				m = h.extend(true,
						{
							url : o,
							success : h.ajaxSettings.success,
							type : l || h.ajaxSettings.type,
				...
				function v(q) {
					...
					m.data = null;
					var M = h.extend(true, {}, h.ajaxSettings, m, {
						contentType : false,
						processData : false,
						cache : false,
						type : l || "POST"
					...
					if (m.uploadProgress) {
						M.xhr = function() {
							...
							if (O.upload) {
								O.upload.addEventListener("progress", function(
										S) {
									var R = 0;
				...
				function I(ah) {
					...
					if (X.beforeSend
							&& X.beforeSend.call(X.context, Q, X) === false) {
						if (X.global) {
							h.active--
						}
						ak.reject();
					...
					function U(an) {
						...
						try {
							am = an.contentDocument ? an.contentDocument
									: an.document
						} catch (al) {
							...
							if (X.extraData) {
								for ( var ar in X.extraData) {
									if (X.extraData.hasOwnProperty(ar)) {
										if (h.isPlainObject(X.extraData[ar])
												&& X.extraData[ar]
														.hasOwnProperty("name")
												&& X.extraData[ar]
					...
					function Z(ar) {
						...
						try {
							...
							var ao = X.dataType == "xml" || aj.XMLDocument
									|| h.isXMLDoc(aj);
							f("isXml=" + ao);
							if (!ao && window.opera
							...
							Q.responseXML = aj.XMLDocument ? aj.XMLDocument
									: aj;
							if (ao) {
								X.dataType = "xml"
							}
							...
							if (av) {
								Q.status = Number(av.getAttribute("status"))
										|| Q.status;
								Q.statusText = av.getAttribute("statusText")
										|| Q.statusText
							}
							...
							if (at || X.textarea) {
								...
								if (aq) {
									...
									Q.status = Number(aq.getAttribute("status"))
											|| Q.status;
									Q.statusText = aq
									...
									if (at) {
										...
										if (am) {
											Q.responseText = am.textContent ? am.textContent
													: am.innerText
										} else {
											if (aw) {
												Q.responseText = aw.textContent ? aw.textContent
														: aw.innerText
											}
										}
									}
						...
						if (Q.status) {
							ap = (Q.status >= 200 && Q.status < 300 || Q.status === 304) ? "success"
									: "error"
						}
						if (ap === "success") {
							if (X.success) {
					...
					var Y = h.parseXML
							|| function(al, am) {
								if (window.ActiveXObject) {
									...
									am = (new DOMParser()).parseFromString(al,
											"text/xml")
								}
								return (am && am.documentElement && am.documentElement.nodeName != "parsererror") ? am
										: null
							};
					...
					var k = function(aq, ao, an) {
						var am = aq.getResponseHeader("content-type") || "", al = ao === "xml"
								|| !ao && am.indexOf("xml") >= 0, ap = al ? aq.responseXML
								: aq.responseText;
			...
			h.fn.ajaxForm = function(i) {
				...
				if (i.delegation) {
					h(document).off("submit.form-plugin", this.selector, d)
							.off("click.form-plugin", this.selector, c).on(
									"submit.form-plugin", this.selector, i, d)
							.on("click.form-plugin", this.selector, i, c);
					return this
				}
				return this.ajaxFormUnbind().bind("submit.form-plugin", i, d)
						.bind("click.form-plugin", i, c)
			};
			function d(j) {
				var i = j.data;
			...
			h.fieldValue = function(j, q) {
				...
				if (q
						&& (!l || j.disabled || x == "reset" || x == "button"
								|| (x == "checkbox" || x == "radio")
								&& !j.checked
								|| (x == "submit" || x == "image") && j.form
								&& j.form.clk != j || y == "select"
								&& j.selectedIndex == -1)) {
					return null
				}
				if (y == "select") {
					...
					for ( var m = (o ? r : 0); m < s; m++) {
						...
						if (p.selected) {
							var w = p.value;
							if (!w) {
								w = (p.attributes && p.attributes.value && !(p.attributes.value.specified)) ? p.text
										: p.value
							}
							if (o) {
								return w
